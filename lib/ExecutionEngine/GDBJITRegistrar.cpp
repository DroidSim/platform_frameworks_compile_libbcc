...
// Global path to write dump output (for debugging the JIT engine)
...
void GDBJITRegistrar::registerObject(const ObjectBuffer* Object, std::size_t Size) {
  ...
  assert(ObjectBufferMap.find(Object) == ObjectBufferMap.end() && "Second attempt to perform debug registration.");
  ...
  if (JITCodeEntry == 0) {
    ...
    // DO NOT MERGE, only for debugging GDB JIT registrations
